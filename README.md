...
Служба доступна **любой веб-странице** (CORS, как и оригинал):

- Команды чтения (`ENUMCERTS`, `GETCERTBODY`, `GETLOGINFO`) отдают данные
  сертификатов (ФИО, ИНН, серийники, отпечатки, DER) любому origin — без
  запроса PIN. Это поведение оригинала; учитывайте при работе на чужих сайтах.
- Подпись/расшифрование по умолчанию подтверждаются PIN-диалогом КриптоПро.
  Сохранённый PIN (`--pin` / `STEK_CSP_PIN`) убирает это подтверждение — тогда
  любая страница может подписать/расшифровать вашим ключом.
- На backend **cryptcp** сохранённый PIN виден в `/proc/<pid>/cmdline`
  локальным пользователям на время операции. Если нужен сохранённый PIN —
  используйте backend **pycades** (PIN остаётся в процессе) или не сохраняйте PIN.
